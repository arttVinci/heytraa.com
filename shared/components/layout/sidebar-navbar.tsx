...
export function SidebarNavbar() {
  ...
  const renderNavContent = () => (
    <div className="flex flex-col items-center w-full">
      ...
      {/* Name & Handle */}
      <div className="flex flex-col items-center mt-3.5 text-center">
        <div className="flex items-center gap-1.5">
          <h2 className="text-lg font-bold tracking-tight text-slate-900">
            Putra Rizky
          </h2>
          <BadgeCheck className="w-5 h-5 text-[#0284c7] fill-[#0284c7] shrink-0" />
        </div>
        <span className="text-xs font-semibold text-[#0284c7]/80 mt-0.5 font-mono">
          @traa_rzkyy
        </span>
      </div>

      {/* Bio / Tagline */}
      <p className="text-[12px] text-slate-500 font-mono tracking-tight text-center mt-2 px-2 leading-relaxed">
      ...
      {/* Nav Menu */}
      <nav className="flex flex-col w-full gap-1.5" aria-label="Main Navigation">
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "bg-gradient-to-r from-[#0284c7] to-[#0ea5e9] text-white shadow-md shadow-sky-500/25 font-semibold"
            ...
            >
              ...
              {isActive && (
                <ChevronRight className="w-4 h-4 text-white/90 animate-in fade-in slide-in-from-left-1 duration-200" />
              )}
            </Link>
          );
        })}
      ...
      <div className="w-full mt-5">
        <button
          ...
          className={cn(
            "relative w-full group overflow-hidden flex items-center justify-center gap-2 px-4 py-3 rounded-2xl",
            "bg-gradient-to-r from-[#0284c7] via-[#0ea5e9] to-[#38bdf8] text-white text-sm font-semibold tracking-wide",
            "shadow-[0_4px_20px_-3px_rgba(2,132,199,0.45)] hover:shadow-[0_6px_25px_-2px_rgba(2,132,199,0.65)]",
        ...
        >
          ...
          <Sparkles className="w-3.5 h-3.5 text-sky-100 ml-0.5 opacity-90" />
        </button>
      </div>
