    ...
    >
      <g>
        ...
        <path
          ...
          className="stroke-white dark:stroke-slate-900 transition-colors"
          strokeWidth={2.8}
        />
      </g>
    </svg>
...
export function SmoothCursor({
  ...
  useEffect(() => {
    ...
    const smoothPointerMove = (e: PointerEvent) => {
      ...
      // Safely check if hovering over clickable element
      const target =
      ...
      const isClickable = Boolean(
        typeof target?.closest === "function" &&
          target.closest(
            'a, button, [role="button"], input, select, textarea, label, [data-interactive]'
          )
      );
      ...
      if (speed > 0.1) {
        ...
        if (!isInteracting.current) {
          scale.set(isClickable ? 1.15 : 0.94);
        }
