...
export interface InfiniteSpiralProps {
  ...
  grayscale?: number;
  className?: string;
  onItemClick?: (item: InfiniteSpiralItem, index: number) => void;
}
...
const InfiniteSpiral = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (!visibleRef.current || Math.abs(deltaScroll) < 0.1) return;
      targetProgressRef.current += clamp(
        (deltaScroll * scrollSpeedMultiplier) /
    ...
    const render = (time: number) => {
      ...
      const fit = Math.min(
        1,
        width / Math.max(radius * 1.8 + cardWidth * 0.7, 1),
        height / Math.max(cardHeight * 2.2, 1),
      );
  ...
  const itemClassName =
    "absolute left-1/2 top-1/2 block h-[var(--spiral-height)] w-[var(--spiral-width)] overflow-hidden rounded-[var(--spiral-radius)] border border-white/20 bg-slate-900 shadow-[0_18px_45px_rgba(0,0,0,0.35)] [backface-visibility:hidden] [transform-style:preserve-3d] [will-change:transform,opacity] motion-reduce:transition-none";
